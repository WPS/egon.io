...
export default function DomainStoryRenderer(eventBus, styles, canvas, textRenderer, pathMap, commandStack) {
  ...
  function renderNumber(parentGfx, label, options, type) {
    ...
    if (/:activity$/.test(type)) {
      number.innerHTML = manipulateInnerHTMLXLabel(number.children, options.box.x, 0);
      number.innerHTML = manipulateInnerHTMLYLabel(number.children, options.box.y, 0);
    } else if (/:actor/.test(type)) {
      height = parentGfx.firstChild.attributes.height.nodeValue;
      number.innerHTML = manipulateInnerHTMLYLabel(number.children, height, 0);
    } else if (/:workObject/.test(type)) {
      height = parentGfx.firstChild.attributes.height.nodeValue;
      number.innerHTML = manipulateInnerHTMLYLabel(number.children, height, 26);
    }

    svgAppend(parentGfx, number);
    return number;
  }
